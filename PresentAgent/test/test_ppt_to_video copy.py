...
def _generate_tts_audio(text: str, output_path: str) -> None:
    try:
        ...
        assets_dir = Path(__file__).resolve().parents[1] / "presentagent" / "MegaTTS3" / "assets"
        speaker_a_prompt = Path(
            os.getenv("TTS_SPEAKER_A_PROMPT", str(assets_dir / "English_prompt.wav"))
        )
        speaker_b_prompt = Path(
            os.getenv("TTS_SPEAKER_B_PROMPT", str(assets_dir / "Chinese_prompt.wav"))
        )
