...
class PPTGen(ABC):
    ...
    def _should_use_video(
        ...
        return bool(result.get("use_video", False))
...
class PPTAgent(PPTGen):
    ...
    def _generate_notes(
        ...
        if not notes:
            return ""
        discussion = self._generate_discussion(slide_content, slide_description)
        return self._merge_notes_with_discussion(notes, discussion)
    def _edit_slide(
...
class PPTAgentAsync(PPTGenAsync):
    ...
    async def _generate_notes(
        ...
        if not notes:
            return ""
        _, discussion_decision = await self.staffs["discussion_judge"](
            slide_content=slide_content,
            slide_description=slide_description,
        )
        discussion = ""
        if discussion_decision.get("need_discussion", False):
            _, discussion = await self.staffs["discussion_writer"](
                slide_content=slide_content,
                slide_description=slide_description,
            )
            question = str(discussion.get("question", "")).strip()
            draft_answer = str(discussion.get("answer", "")).strip()
            if question:
                _, answer_payload = await self.staffs["discussion_answerer"](
                    document_overview=self.source_doc.get_overview(include_summary=True),
                    slide_content=slide_content,
                    slide_description=slide_description,
                    question=question,
            ...
            if question and answer:
                discussion = f"Speaker A: {question}\nSpeaker B: {answer}"
            else:
                discussion = ""
        return self._merge_notes_with_discussion(notes, discussion)
