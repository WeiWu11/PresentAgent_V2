...
from .element import Media, Section, SubSection, Table, link_medias
...
def to_paragraphs(original_text: str, max_chunk_size: int = 256):
    ...
    for i, para in enumerate(original_text.split("\n\n")):
        ...
        if MARKDOWN_TABLE_REGEX.match(para):
            ...
            medias.append(paragraph)
        elif HTML_VIDEO_REGEX.match(para):
...
class Document:
    ...
    def iter_medias(self, media_type: Optional[str] = None):
    ...
    def find_media(self, caption: str, media_type: Optional[str] = None):
        for media in self.iter_medias(media_type):
            if media.caption == caption:
                return media
        media_label = media_type or "media"
        raise ValueError(f"{media_label.capitalize()} caption not found: {caption}")

    def find_caption(self, caption: str):
        return self.find_media(caption, "image").path
...
class OutlineItem:
    ...
    def retrieve(self, slide_idx: int, document: Document):
        ...
        for subsection in subsections:
            for media in subsection.medias:
                if media.media_type != "video" or media.path is None:
                    continue
                if media.path in seen_video_paths:
                    continue
                seen_video_paths.add(media.path)
                videos.append(
                    f"Video: {media.path}\nCaption: {media.caption or media.path}"
                )
        return header, content, images, videos
    ...
    def check_images(self, document: Document, text_model: LLM, sim_bound: float):
        doc_images = list(document.iter_medias("image"))
        image_embeddings = []
