...
class Picture(ShapeElement):
    ...
    def build(self, slide: PPTXSlide) -> PPTXPicture:
        ...
        if self.is_video:
            add_movie = getattr(slide.shapes, "add_movie", None)
            if add_movie is None:
                raise ValueError(
                    "python-pptx does not support add_movie in this environment."
                )
            mime_type = guess_type(self.img_path)[0] or "video/unknown"
            shape = add_movie(
                self.img_path,
                **self.style["shape_bounds"],
                poster_frame_image=package_join("resource", "pic_placeholder.png"),
                mime_type=mime_type,
            )
    ...
    def is_video(self) -> bool:
        return self.style.get("is_video", False)

    @is_video.setter
    def is_video(self, value: bool) -> None:
        self.style["is_video"] = value
    ...
    def to_html(self, style_args: StyleArg) -> str:
        ...
        if self.is_video:
            return (
                self.indent
                + f"<video {self.get_inline_style(style_args)} src='{self.img_path}' title='{self.caption}'></video>"
            )
        return (
            self.indent
