...
VIDEO_REGEX = re.compile(r".*\.(mp4|mov|webm|m4v)$", re.IGNORECASE)
...
def replace_image(slide: SlidePage, doc: Document, img_id: int, image_path: str):
    ...
    """
    if VIDEO_REGEX.match(image_path):
        replace_video(slide, img_id, image_path)
        return
    ...
    shape.is_video = False


def replace_video(slide: SlidePage, img_id: int, video_path: str):
    """
    Replace a picture placeholder with an embedded video.

    Args:
        slide (SlidePage): The slide containing the visual placeholder.
        img_id (int): The ID of the picture placeholder to replace.
        video_path (str): The local path to the video file.

    Raises:
        SlideEditError: If the target element is not a picture or the video file is missing.
    """
    if not os.path.exists(video_path):
        raise SlideEditError(f"The video {video_path} does not exist.")
    shape = element_index(slide, img_id)
    if not isinstance(shape, Picture):
        raise SlideEditError(
            f"The element {shape.shape_idx} of slide {slide.slide_idx} is not a Picture."
        )
    shape.img_path = video_path
    shape.is_video = True
