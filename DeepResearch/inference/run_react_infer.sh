...
CUDA_VISIBLE_DEVICES=$VLLM_GPU_ID vllm serve "$MODEL_PATH" --host 0.0.0.0 --port "$VLLM_PORT" --gpu-memory-utilization 0.65 --max-num-seqs 16 --disable-log-requests &
...
echo "Mode: Single-GPU main model"
...
python -u run_multi_react.py --dataset "$DATASET" --output "$OUTPUT_PATH" --max_workers $MAX_WORKERS --model $MODEL_PATH --temperature $TEMPERATURE --presence_penalty $PRESENCE_PENALTY --total_splits ${WORLD_SIZE:-1} --worker_split $((${RANK:-0} + 1)) --roll_out_count 3
